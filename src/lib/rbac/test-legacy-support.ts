...
async function testLegacyTokenSupport() {
  ...
  try {
    ...
    const validation =
      await LegacyTokenSupport.validateLegacyToken(legacyToken);
    console.log(
      `✅ Legacy token validation: ${validation.isValid ? 'VALID' : 'INVALID'}`
    );
    ...
    const shouldUpgrade =
      await LegacyTokenSupport.shouldUpgradeToken(legacyToken);
    ...
    const legacyContext =
      await LegacyTokenSupport.extractLegacyContext(legacyToken);
    ...
    try {
      ...
      if (upgrade.success) {
        ...
        console.log(
          `   - New token generated: ${upgrade.newToken ? 'YES' : 'NO'}`
        );
      ...
      console.log(
        `⚠️  Legacy token upgrade test failed (expected): ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
    ...
    console.log(
      `   - Total legacy tokens used: ${stats.totalLegacyTokensUsed}`
    );
    console.log(`   - Total upgrades: ${stats.totalUpgrades}`);
    console.log(
      `   - Recent activity: ${stats.recentLegacyActivity.length} events`
    );
    ...
    console.log(
      `   - Stats: ${report.stats.totalLegacyTokensUsed} used, ${report.stats.totalUpgrades} upgraded`
    );
    ...
    const invalidValidation =
      await LegacyTokenSupport.validateLegacyToken('invalid-token');
    console.log(
      `✅ Invalid token validation: ${invalidValidation.isValid ? 'VALID' : 'INVALID'}`
    );
    ...
    console.log(
      `✅ Empty token validation: ${emptyValidation.isValid ? 'VALID' : 'INVALID'}`
    );
