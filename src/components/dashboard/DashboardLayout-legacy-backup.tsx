...
interface User {
  ...
  mustChangePassword?: boolean; // For staff and restaurant owners
  ...
  const fetchCurrentUser = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Check if user must change password (staff or restaurant owner)
        if ((userData.userType === 'staff' || userData.userType === 'restaurant_owner') && userData.mustChangePassword === true) {
        ...
        if (userData.userType === 'restaurant_owner' && userData.restaurants?.length > 0) {
          // Check if owner has any active restaurants
          const activeRestaurants = userData.restaurants.filter(r => r.isActive);
          if (activeRestaurants.length === 0) {
            // No active restaurants - redirect to owner dashboard
