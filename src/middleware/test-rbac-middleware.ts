...
import { EnhancedAuthenticatedUser } from '@/lib/rbac/types';
...
async function testRBACMiddleware() {
  ...
  try {
    ...
    console.log(
      `✅ No token test: ${noTokenResult.isAuthorized ? 'FAILED' : 'PASSED'}`
    );
    ...
    const invalidTokenResult = await RBACMiddleware.protect(
      requestInvalidToken,
      {
        auditLog: true,
      }
    ...
    console.log(
      `✅ Invalid token test: ${invalidTokenResult.isAuthorized ? 'FAILED' : 'PASSED'}`
    );
    ...
    if (userRoles.length === 0) {
      console.log(
        '   ⚠️  No user roles found - skipping token validation tests'
      );
      ...
      const testToken = await EnhancedJWTService.generateToken(
        testUser as EnhancedAuthenticatedUser,
      ...
      console.log(
        `✅ Valid token test: ${validTokenResult.isAuthorized ? 'PASSED' : 'FAILED'}`
      );
      if (validTokenResult.isAuthorized && validTokenResult.context) {
        console.log(`   - User ID: ${validTokenResult.context.user.id}`);
        console.log(
          `   - User Type: ${validTokenResult.context.user.userType}`
        );
        console.log(
          `   - Role: ${validTokenResult.context.currentRole.roleTemplate}`
        );
        console.log(
          `   - Permissions: ${validTokenResult.context.permissions.length}`
        );
      }
      ...
      const userTypeRestrictResult = await RBACMiddleware.protect(
        requestValidToken,
        {
      ...
      const testPassed = shouldBeDenied
        ? !userTypeRestrictResult.isAuthorized
        : userTypeRestrictResult.isAuthorized;

      console.log(
        `✅ User type restriction test: ${testPassed ? 'PASSED' : 'FAILED'}`
      );
      console.log(`   - Current user type: ${testRole.userType}`);
      console.log(`   - Should be denied: ${shouldBeDenied}`);
      console.log(
        `   - Actually authorized: ${userTypeRestrictResult.isAuthorized}`
      );

      // Test 6: Role restriction
      console.log('\n6. Testing role restriction...');

      const roleRestrictResult = await RBACMiddleware.protect(
        requestValidToken,
        {
      ...
      const roleTestPassed = shouldBeDeniedRole
        ? !roleRestrictResult.isAuthorized
        : roleRestrictResult.isAuthorized;

      console.log(
        `✅ Role restriction test: ${roleTestPassed ? 'PASSED' : 'FAILED'}`
      );
      console.log(`   - Current role: ${testRole.roleTemplate}`);
      console.log(`   - Should be denied: ${shouldBeDeniedRole}`);
      console.log(
        `   - Actually authorized: ${roleRestrictResult.isAuthorized}`
      );

      // Test 7: Permission restriction
      console.log('\n7. Testing permission restriction...');

      const permissionRestrictResult = await RBACMiddleware.protect(
        requestValidToken,
        {
      ...
      console.log(
        `✅ Permission restriction test: ${!permissionRestrictResult.isAuthorized ? 'PASSED' : 'FAILED'}`
      );
      console.log(`   - Required permission: non-existent-permission`);
      console.log(
        `   - User permissions: ${validTokenResult.context?.permissions.join(', ')}`
      );
      ...
      console.log(
        `✅ Authorization header test: ${authHeaderResult.isAuthorized ? 'PASSED' : 'FAILED'}`
      );
      console.log(
        `   - Token extracted from header: ${authHeaderResult.isAuthorized ? 'YES' : 'NO'}`
      );
      ...
      const rateLimitResult1 = await RBACMiddleware.protect(
        requestValidToken,
        rateLimitConfig
      );
      console.log(
        `   - Request 1: ${rateLimitResult1.isAuthorized ? 'PASSED' : 'FAILED'}`
      );

      // Second request should pass
      const rateLimitResult2 = await RBACMiddleware.protect(
        requestValidToken,
        rateLimitConfig
      );
      console.log(
        `   - Request 2: ${rateLimitResult2.isAuthorized ? 'PASSED' : 'FAILED'}`
      );

      // Third request should be rate limited
      const rateLimitResult3 = await RBACMiddleware.protect(
        requestValidToken,
        rateLimitConfig
      );
      console.log(
        `   - Request 3: ${!rateLimitResult3.isAuthorized ? 'PASSED (rate limited)' : 'FAILED'}`
      );
    ...
    configNames.forEach((configName) => {
      const config =
        rbacMiddlewareConfigs[configName as keyof typeof rbacMiddlewareConfigs];
      console.log(
        `   - ${configName}: ${JSON.stringify(config).substring(0, 100)}...`
      );
    });
    ...
    console.error('❌ RBAC Middleware test failed:', _error);
