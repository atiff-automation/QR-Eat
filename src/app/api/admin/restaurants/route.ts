  ...
  try {
    ...
    if (existingSlug) {
      return NextResponse.json(
        {
          error:
            'Restaurant slug already exists. Please choose a different one.',
        },
        { status: 409 }
      );
    }
    ...
    return NextResponse.json({
      ...
      instructions:
        'The owner will receive an email with login credentials and must change the password on first login.',
      emailSent: true,
    });
  } catch (error) {
