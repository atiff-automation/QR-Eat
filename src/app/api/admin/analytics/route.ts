...
export async function GET(request: NextRequest) {
  try {
    ...
    const monthsToShow =
      range === '1y' ? 12 : range === '90d' ? 3 : range === '7d' ? 7 : 4;
    ...
    for (let i = monthsToShow - 1; i >= 0; i--) {
      ...
      if (range === '1y') {
        ...
        monthStart.setDate(
          monthStart.getDate() - (i + 1) * (range === '7d' ? 1 : 7)
        );
      ...
      revenueByMonth.push({
        month:
          range === '7d'
            ? monthStart.toLocaleDateString('en-US', { weekday: 'short' })
            : range === '1y'
    ...
    const restaurantRevenue = restaurants
      .map((restaurant) => {
        ...
        const previousRestaurantOrders = previousOrders.filter(
          (order) => order.restaurantId === restaurant.id
        );
        const growth =
          previousRestaurantOrders.length > 0
            ? ((restaurantOrders.length - previousRestaurantOrders.length) /
                previousRestaurantOrders.length) *
              100
            : 0;
    ...
    const activeRestaurantsRate =
      restaurants.length > 0
        ? (activeRestaurants / restaurants.length) * 100
        : 0;
